...
function Navbar() {
  ...
  return (
    <div>
        <header class="border border-grey-100 relative flex flex-wrap sm:justify-start sm:flex-nowrap w-full bg-white text-sm py-3">
    ...
    <div class="flex items-center justify-between">
      ...
      <a class="flex-none mx-4 text-xl font-semibold focus:outline-none focus:opacity-80" href="#" aria-label="Brand">
      YourHR
        </a>
      <div class="sm:hidden">
      ...
      <div class="flex flex-col gap-5 mt-5 sm:flex-row sm:items-center sm:justify-end sm:mt-0 sm:ps-5">
        {
            localStorage.getItem('token') ? (
                <>
                <a class="text-gray-800 hover:text-gray-900" onClick={handleLogout} aria-label="Logout">Logout</a>
                </>
            ) : (
                <>
                <a class="text-gray-800 hover:text-gray-900" href="/login" aria-label="Login">Login</a>
                <a class="text-gray-800 hover:text-gray-900" href="/signup" aria-label="Sign up">Sign up</a>
                </>
            )
        }
